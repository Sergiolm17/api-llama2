...
vectorstore_chroma = Chroma(
    persist_directory=NOMBRE_INDICE_CHROMA, embedding_function=embedding_instruct
)
...
# API route to generate query
@app.route("/search", methods=["POST"])
def search():
    if request.method == "POST":
        # Obtener el JSON de entrada enviado por el cliente
        data = request.get_json()

        # Verificar si el JSON contiene una clave llamada 'query'
        if "query" in data:
            query = data["query"]
            ...
            result = {
                "query": query,
                "most_similar_doc": docs_q[0]
                if docs_q
                else None,  # El documento más similar (si hay resultados)
            }

            return jsonify(result)
        else:
            # Si el JSON de entrada no contiene la clave 'query', retorna un mensaje de error
            return (
                jsonify(
                    {
                        "error": 'El JSON de entrada debe contener una clave llamada "query"'
                    }
                ),
                400,
            )
